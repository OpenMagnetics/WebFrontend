...
import { defaultDesignRequirements, designRequirementsOrdered, isolationSideOrdered } from '/src/assets/js/defaults.js'
    ...
    methods: {
        ...
        updatedNumberElements(newLength, name) {
            if (name == 'numberWindings') {
                this.masStore.mas.inputs.designRequirements.turnsRatios;
                const newElements = [];
                for (var i = 0; i < newLength - 1; i++) {
                    if (i < this.masStore.mas.inputs.designRequirements.turnsRatios.length) {
                        newElements.push(this.masStore.mas.inputs.designRequirements.turnsRatios[i]);
                    }
                    else {
                        newElements.push({'nominal': 1});
                    }
                }
                this.masStore.mas.inputs.designRequirements.turnsRatios = newElements;
    ...
    <div class="container">
        <div v-tooltip="styleTooltip" class="row">
            <div class="col-sm-12 col-md-4 text-start border border-primary" style="max-width: 360px;">
                <div class="my-3" v-for="requirementName in designRequirementsOrdered" >
                    <label v-tooltip="tooltipsMagneticSynthesisDesignRequirements[requirementName]"  class="rounded-2 fs-5 ms-3 ">{{toTitleCase(shortenedLabels[requirementName])}}</label>
                
                    <button v-if="!compulsoryRequirements.includes(requirementName)" :class="masStore.mas.inputs.designRequirements[requirementName]==null? 'btn-info' : 'btn-danger'" class="btn float-end" :style="'filter: brightness(70%)'" @click="requirementButtonClicked(requirementName)">
                        {{masStore.mas.inputs.designRequirements[requirementName]==null? 'Add Req.' : 'Remove'}}
                    </button>
                    <button v-if="compulsoryRequirements.includes(requirementName)" class="btn btn-light float-end disabled">
                        Required
                    </button>
                </div>
            </div>
            <div class="col-sm-12 col-md-8 text-start pe-0">
                ...
                <DimensionWithTolerance class="border-bottom py-2" :name="'magnetizingInductance'" v-if="masStore.mas.inputs.designRequirements.magnetizingInductance != null" :defaultValue="defaultDesignRequirements.magnetizingInductance" v-model="masStore.mas.inputs.designRequirements.magnetizingInductance" unit="H"/>
                ...
                <ArrayDimensionWithTolerance class="border-bottom py-2" :name="'leakageInductance'" v-if="masStore.mas.inputs.designRequirements.leakageInductance != null" defaultField="maximum" :defaultValue="defaultDesignRequirements.leakageInductance[0]" unit="H" :fixedNumberElements="masStore.mas.inputs.designRequirements.turnsRatios.length"/>

                <ArrayDimensionWithTolerance class="border-bottom py-2" :name="'strayCapacitance'" v-if="masStore.mas.inputs.designRequirements.strayCapacitance != null" defaultField="maximum" :defaultValue="defaultDesignRequirements.strayCapacitance[0]" unit="F" :fixedNumberElements="masStore.mas.inputs.designRequirements.turnsRatios.length"/>

                <DimensionWithTolerance class="border-bottom py-2" :name="'operatingTemperature'" v-if="masStore.mas.inputs.designRequirements.operatingTemperature != null" :defaultValue="defaultDesignRequirements.operatingTemperature" v-model="masStore.mas.inputs.designRequirements.operatingTemperature" unit="°C"/>
              
                <MaximumWeight class="border-bottom py-2" v-model="masStore.mas.inputs.designRequirements" v-if="masStore.mas.inputs.designRequirements.maximumWeight != null" :defaultValue="300" unit="g"/>

                <MaximumDimensions class="border-bottom py-2" v-if="masStore.mas.inputs.designRequirements.maximumDimensions != null" :defaultValue="defaultDesignRequirements.maximumDimensions" unit="m" v-model="masStore.mas.inputs.designRequirements.maximumDimensions"/>

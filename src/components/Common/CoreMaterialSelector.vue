        ...
        <div class="row">
            ...
            <div class="col-8">
                <div class="container-flex p-0 m-0">
                    <div class="row">
                        <select :data-cy="dataTestLabel + '-material-select-input'" name="quickMaterialField" ref="quickMaterialFieldRef" @change="$emit('onMaterialChange', $event.target.value)" class= "fs-6 bg-light text-white rounded-2 col-8 m-0 p-0" v-model="materialSelected">
                            <option :data-cy="dataTestLabel + '-material-NA-option-input'" disabled value="">Please select one</option>
                            <option :data-cy="dataTestLabel + '-material-custom-option-input'" disabled value="Custom">Custom</option>
                            <option v-for="model, index in commercialMaterialNames"
                                :data-cy="dataTestLabel + '-material-' + model + '-option-input'"
                                :disabled="materialManufacturers.includes(model)"
                                :key="index"
                                :value="model">{{model}}
                            </option>
                        </select>
                        <button :data-cy="dataTestLabel + '-material-table-modal-button'" v-tooltip="'Open information table for materials'" class="btn btn-primary text-dark py-1 p-0 px-0 mx-1 offset-1 col-2" data-bs-toggle="modal" data-bs-target="#loadCommercialMaterialModal" >

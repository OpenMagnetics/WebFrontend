            ...
            for (var operatingPointIndex = 0; operatingPointIndex < data.outputs.length; operatingPointIndex++) {
                ...
                for (var windingIndex = 0; windingIndex < data.magnetic.coil.functionalDescription.length; windingIndex++) {
                    ...
                    }
                    {
                        ...
                        const wireString = JSON.stringify(this.masStore.mas.magnetic.coil.functionalDescription[windingIndex].wire);
                        const currentString = JSON.stringify(this.masStore.mas.inputs.operatingPoints[0].excitationsPerWinding[windingIndex].current);

                        const currentDensity = this.$mkf.calculate_effective_current_density(wireString, currentString, this.masStore.mas.inputs.operatingPoints[0].conditions.ambientTemperature) / this.masStore.mas.magnetic.coil.functionalDescription[windingIndex].numberParallels;

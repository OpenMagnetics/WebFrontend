        ...
        <div class="modal-dialog">
            <div class="modal-content bg-dark text-white">
                <div class="modal-header">
                    ...
                    <button data-test="OperationPointPublish-corner-close-button" type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="publishOperationPointModalClose"></button>
                </div>
                <div class="modal-body row mt-4">
                    ...
                    <a data-test="OperationPointPublish-slug-link" class="text-primary my-3 offset-1 col-6"  :href="getURL">{{getURL}}</a>
                    ...
                    <Form ref="formRef" :validation-schema="schema" v-slot="{ errors }" class="form-inline row">
                        ...
                        <Field data-test="OperationPointPublish-slug-input" name="slug" type="text" :class="{ 'is-invalid': errors.slug }" class= "small-text bg-light text-white rounded-2 col-sm-8 col-md-8 col-lg-8 col-xl-8" v-model="slug"/>
                    ...
                    <img data-test="OperationPointPublish-loading" v-if="posting" class="mx-auto d-block" alt="loading" style="width: 150px; height: auto;" src="/images/loading.gif">
                    <button data-test="OperationPointPublish-publish-button" v-if="!posting" class="btn text-dark bg-primary mt-2 offset-1 col-5" @click="onPublish" >{{isPublished? "Update" : "Publish"}}</button>
                    <button data-test="OperationPointPublish-close-button" v-if="!posting" class="btn btn-dark text-primary border-primary mx-auto d-block mt-2 offset-1 col-5" data-bs-dismiss="modal" >Close</button>

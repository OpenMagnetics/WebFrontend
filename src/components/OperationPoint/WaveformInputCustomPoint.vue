...
<template>
    <div class="container-flex text-white mt-2 mb-1">
        <Form ref="formRef" :validation-schema="schema" v-slot="{ handleSubmit, errors }" class="form-inline"  @submit="handleSubmit($event, onSubmit)">
            ...
            <Field :data-cy="dataTestLabel + '-time-input'" name="timeValidator" type="number" :disabled="index == 0 || index == (store.getDataPoints.value.length - 1)" :value="timeVar" @change="onTimeChange" :class="{ 'is-invalid': errors.timeValidator }" class="rounded-2 bg-light text-white" style="width: 100%; max-width: 50px;"/>
            ...
            <Field :data-cy="dataTestLabel + '-value-input'" name="valueValidator" type="number" :value="valueVar" @change="onValueChange" :class="{ 'is-invalid': errors.valueValidator }" class=" rounded-2 bg-light text-white" style="width: 100%; max-width: 60px;"/>
            <label class="fs-5 mx-1" style="width: 10px;">{{electricalParameter == "current"? 'A' : 'V'}}</label>
            <span class="ms-2">
                <button :data-cy="dataTestLabel + '-add-point-below-button'" v-if="index != (store.getDataPoints.value.length - 1)" type="button" class="btn btn-default btn-circle bg-dark mb-1 me-1" @click="onAddPointBelow"><i class="fa-solid fa-circle-plus text-secondary"></i> </button>
                <button :data-cy="dataTestLabel + '-remove-point-button'" v-if="index != 0 && index != (store.getDataPoints.value.length - 1)" type="button" class="btn btn-default btn-circle bg-dark mb-1" @click="onRemovePoint"><i class="fa-solid fa-circle-minus text-danger"></i> </button>
            </span>

...
<template>
    ...
    <div class="offcanvas-header">
        ...
        <button data-test-id="OperationPointImport-corner-close-modal-button" type="button" class="btn-close btn-close-white" data-bs-dismiss="offcanvas" aria-label="ImportOffCanvasClose"></button>
    </div>
    <div class="offcanvas-body">
        <div class="mt-2">
            ...
            <div class="btn-group-vertical my-3" role="group" aria-label="Vertical button group">
                <button data-test-id="OperationPointImport-source-MAS-button" type="button" @click="onFileTypeSelected" class="btn btn-primary mt-1 rounded-3">Magnetic Agnostic Structure file</button>
                <button data-test-id="OperationPointImport-source-CSV-button" type="button" class="btn btn-primary mt-1 rounded-3 disabled">CSV file (coming soon)</button>
                <button data-test-id="OperationPointImport-source-Simba-button" type="button" class="btn btn-primary mt-1 rounded-3 disabled">Simba (coming soon)</button>
                <button data-test-id="OperationPointImport-source-LTSPice-button" type="button" class="btn btn-primary mt-1 rounded-3 disabled">LTSpice (coming soon)</button>
                <button data-test-id="OperationPointImport-source-PSIM-button" type="button" class="btn btn-primary mt-1 rounded-3 disabled">PSIM (coming soon)</button>
            </div>
        </div>

        <label class="fs-5 text-white mt-3 mb-2"> {{fileType != null}} </label>
        <label class="fs-5 text-white mt-3 mb-2"> {{fileType == 'MAS'}} </label>
        <label class="fs-5 text-white mt-3 mb-2"> {{isCompressed}} </label>
        <div v-if="fileType != null">
            ...
            <div class="mt-2" v-if="!isCompressed">
                ...
                <div class="form-check">
                    <input data-test-id="OperationPointImport-do-compress-waveform-radio" class="form-check-input" type="radio" name="tryCompressWaveforms" id="doTryCompressWaveforms" value="compressWaveforms" checked v-model="tryCompressWaveformsPicked">
                    <label class="form-check-label text-white" for="doTryCompressWaveforms">
                        Yes, reduce the number of points in the graph
                    </label>
                </div>
                <div class="form-check">
                    <input data-test-id="OperationPointImport-dont-compress-waveform-radio" class="form-check-input" type="radio" name="tryCompressWaveforms" id="dontTryCompressWaveformsBoth" value="keepUncompressedWaveforms" v-model="tryCompressWaveformsPicked">
        ...
        <button data-test-id="OperationPointImport-import-button" class="mt-5 btn text-light bg-primary float-start fs-5 px-4" data-bs-dismiss="offcanvas" @click="onImport">Import</button>
    </div>
</div>

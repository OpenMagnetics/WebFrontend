    ...
    <template v-if="psfbDiagnostics" #diagnostics>
      ...
      >
        ...
        <tbody>
          ...
          <tr><td>ZVS Lagging</td><td v-for="(op, i) in psfbDiagnostics.perOp" :key="i" :style="{ textAlign: 'right', padding: '2px 4px', color: op.zvsMarginLagging <= 0 ? '#e0b020' : 'inherit' }">{{ Number(op.zvsMarginLagging).toFixed(3) }}</td></tr>

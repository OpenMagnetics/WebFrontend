    ...
    methods: {
        ...
        async simulateIdealWaveforms() {
            ...
            try {
                ...
                // Debug: Log waveform counts
                console.log('IsolatedBuckBoost simulation complete:');
                console.log('  Magnetic waveforms:', this.magneticWaveforms.length, 'operating points');
                console.log('  Converter waveforms:', this.converterWaveforms.length, 'operating points');
                if (this.converterWaveforms.length > 0 && this.converterWaveforms[0].waveforms) {
                    console.log('  Converter waveforms[0] contains', this.converterWaveforms[0].waveforms.length, 'waveforms:');
                    this.converterWaveforms[0].waveforms.forEach((wf, idx) => {
                        console.log('    [' + idx + ']', wf.label, '(' + wf.unit + ')');
                    });
                }
        ...
        buildMagneticWaveformsFromInputs(operatingPoints) {
            ...
            for (let opIdx = 0; opIdx < operatingPoints.length; opIdx++) {
                ...
                if (op.outputVoltages && op.outputVoltages.length > 0 && op.outputVoltages[0].waveform) {
                    const { time, data } = this.repeatWaveformForPeriods(
                        op.outputVoltages[0].waveform.time,
                        op.outputVoltages[0].waveform.data,
                        this.numberOfPeriods
                    );
                    opWaveforms.waveforms.push({
                ...
                if (op.outputCurrents && op.outputCurrents.length > 0 && op.outputCurrents[0].waveform) {
                    const { time, data } = this.repeatWaveformForPeriods(
                        op.outputCurrents[0].waveform.time,
                        op.outputCurrents[0].waveform.data,
                        this.numberOfPeriods
                    );
                    opWaveforms.waveforms.push({
        ...
        getPairedWaveformsList(waveforms, operatingPointIndex, isMagnetic = true) {
            ...
            // Debug: Log waveforms for converter view
            if (!isMagnetic) {
                console.log('IsolatedBuckBoost converter waveforms (' + allWaveforms.length + ' total):', 
                    allWaveforms.map((wf, idx) => ({ idx, label: wf.label, unit: wf.unit })));
            }
            ...
            // Debug: Log pairs for converter view
            if (!isMagnetic) {
                console.log('IsolatedBuckBoost converter pairs (' + pairs.length + ' total):', pairs.map((p, idx) => ({
                    pairIdx: idx,
                    voltage: p.voltage ? p.voltage.wf.label : null,
                    voltageDataPoints: p.voltage ? (p.voltage.wf.y ? p.voltage.wf.y.length : 0) : 0,
                    current: p.current ? p.current.wf.label : null,
                    currentDataPoints: p.current ? (p.current.wf.y ? p.current.wf.y.length : 0) : 0,
                    isVoltagePair: p.isVoltagePair || false
                })));
            }
...
<template>
    <div class="wizard-container container-fluid px-3">
        ...
        <div class="row g-2">
            <!-- Column 1: Design Mode, Design Params, Conditions -->
            <div class="col-12 col-xl-3">
                <div class="d-flex flex-column gap-2">
                    ...
                    <!-- Conditional: Design Parameters (Advanced) OR Current Settings (Help) -->
                    ...
                    <div v-else class="compact-card">
                        <div class="compact-header"><i class="fa-solid fa-microchip me-1"></i>Current Settings</div>
                        <div class="compact-body ps-4">
                            <ElementFromListRadio
                                :name="'currentOptions'"

...
const schema = Yup.object().shape({
    ...
    dutyCycleValidator: Yup.number().required().typeError("The value for duty cycle must be a number").min(0.01).max(100),
});

const formRef = ref(null);
const switchingFrequency = ref(Defaults.defaultSwitchingFrequency / 1000);
const dutyCycle = ref(Defaults.defaultDutyCycle * 100);

    ...
    <div class="col-md-6 col-lg-4 col-xl-3 d-flex align-items-stretch">
        <div class="card mt-4 box-shadow" :class=card_class>
            <div class="card-header bg-transparent row mb-2">
                <p :data-test="dataTestLabel + '-index-text'" class="text-secondary col-2">{{index + 1}}</p>
                <h3 :data-test="dataTestLabel + '-title-text'" class="text-dark col-7 font-size-lg">{{title}}</h3>
                <div class="col-3 ">
                    <span :data-test="dataTestLabel + '-section-text'" :style=badge_color_style class="badge rounded-pill text-wrap font-size-lg">{{section}}</span>
                </div>
            </div>

            <div :data-test="dataTestLabel + '-description-text'" class="card-body" style="display: flex; justify-content: space-between; p-0 m-0">
                <p class="card-text text-start text-dark mb-auto mx-2">{{description}}</p>
                <img :data-test="dataTestLabel + '-image'" v-if="imgSrc != null" class="card-img-right flex-auto d-none d-lg-block" :alt="imgAlt" style="object-fit: cover; width: 150px; max-height: 150px;" :src="imgSrc">
            </div>
            <div v-if="state == 'To Do'" class="card-footer bg-transparent">
                <p :data-test="dataTestLabel + '-votes-text'" class="text-end text-dark my-0">Current votes: {{numberVotesTemp}}
                    <button :data-test="dataTestLabel + '-vote-button'" :disabled="alreadyVoted" :class=btn_class @click="vote">{{alreadyVoted? "Already voted" : "Vote feature!"}}</button>
                </p>
            </div>
            <div v-else class="card-footer bg-transparent">
                <span :data-test="dataTestLabel + '-state-text'" style="background-color: var(--bs-info); color: var(--bs-secondary); font-size: 20px" class="badge rounded-pill text-wrap ">{{state}}</span>
            </div>
        </div>
    </div>
</template>

...
<template>    
    <div v-if="isNotToroid" class="container-flex text-primary mt-2">
        <Form ref="formRef" :validation-schema="schema" v-slot="{ handleSubmit, errors }" class="form-inline row" @submit="handleSubmit($event, onSubmit)">
            ...
            <img data-cy="SimulationCoreCalculatorGappingFromNumberTurnsAndInductance-loading" id="svg" v-if="computing" class="mx-auto d-block col-12" alt="loading" style="width: 25%; height: auto;" src="/images/loading.gif">
            ...
            <label data-cy="SimulationCoreCalculatorGappingFromNumberTurnsAndInductance-gap-length-text" v-if="!computing" class="fs-5 bg-light text-white col-5 text-end">{{gapLength}}</label>
            ...
            <label data-cy="SimulationCoreCalculatorGappingFromNumberTurnsAndInductance-number-gaps-text" v-if="!computing" class="fs-5 bg-light text-white col-5 text-end">{{numberGaps}}</label>

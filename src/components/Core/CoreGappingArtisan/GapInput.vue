...
<template>
    <div v-tooltip="styleTooltip" class="container">
        ...
        <Form ref="formRef" :validation-schema="schema" v-slot="{ errors }" class="form-inline row text-white" @submit="handleSubmit($event, onSubmit)">
            ...
            <Field :data-test-id="'CoreGappingArtisanGapInput-column-' + index + '-gap-type-select-input'" name="gapType" as="select" :class="{ 'is-invalid': errors.gapType }" @change="onGapTypeChange" class= "small-text bg-light text-white rounded-2 mt-2 col-sm-8 col-md-7 col-lg-7 col-xl-7" v-model="gapTypeSelected">
                <option :data-test-id="'CoreGappingArtisanGapInput-column-' + index + '-gap-type-NA-option-input'" disabled value="">Please select one</option>
                <option :data-test-id="'CoreGappingArtisanGapInput-column-' + index + '-gap-type-residual-option-input'" value="Residual">Residual</option>
                <option :data-test-id="'CoreGappingArtisanGapInput-column-' + index + '-gap-type-spacer-option-input'" value="Spacer">Spacer</option>
                <option :data-test-id="'CoreGappingArtisanGapInput-column-' + index + '-gap-type-grinded-option-input'" value="Grinded">Grinded</option>
                <option :data-test-id="'CoreGappingArtisanGapInput-column-' + index + '-gap-type-distributed-option-input'" :disabled="coreStore.distributedGapAlreadyInUse && gapTypeSelected != 'Distributed'" value="Distributed">{{coreStore.distributedGapAlreadyInUse && gapTypeSelected != 'Distributed'? 'Distributed (only one allowed)' : 'Distributed'}}</option>
            </Field>

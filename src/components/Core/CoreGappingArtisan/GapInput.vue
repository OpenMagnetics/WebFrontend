...
<template>
    <div v-tooltip="styleTooltip" class="container">
        ...
        <Form ref="formRef" :validation-schema="schema" v-slot="{ errors }" class="form-inline row text-white" @submit="handleSubmit($event, onSubmit)">
            ...
            <Field :data-cy="'CoreGappingArtisanGapInput-column-' + index + '-gap-type-select-input'" name="gapType" as="select" :class="{ 'is-invalid': errors.gapType }" @change="onGapTypeChange" class= "small-text bg-light text-white rounded-2 mt-2 col-sm-8 col-md-7 col-lg-7 col-xl-7" v-model="gapTypeSelected">

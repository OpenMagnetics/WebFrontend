...
import OperationPointImport from '/src/components/Toolbox/OperatingPoints/OperationPointImport.vue'
...
<template>
    <OperationPointImport @importData="importData"/>
    <div class="container">
        <div class="row" v-tooltip="styleTooltip">
            <div class="col-sm-12 col-md-2 text-start border border-primary m-0 px-1">
                <div class="col-12 row m-0 p-0 border-bottom border-top rounded-4 border-4 mb-5 pb-2 pt-2 mt-2 bg-light" :style="operationPointIndex == currentOperatingPointIndex? 'opacity: 1;' : 'opacity: 0.65;'"  v-for="operationPoint, operationPointIndex in masStore.mas.inputs.operatingPoints">
                    <input :data-cy="dataTestLabel + '-operating-point-' + operationPointIndex + '-name-input'" type="text" class="m-0 px-0 col-12 bg-dark text-white" 
                    ...
                    <div v-if="currentOperatingPointIndex == operationPointIndex" class="col-12 row m-0 p-0" v-for="winding, windingIndex in masStore.mas.magnetic.coil.functionalDescription">
                        <input :data-cy="dataTestLabel + '-operating-point-' + operationPointIndex + '-winding-' + windingIndex + '-name-input'" class="rounded-2 fs-5 ms-2 bg-light text-white col-7 p-0 mb-2 border-0" v-model="winding.name">
                        <button :data-cy="dataTestLabel + '-operating-point-' + operationPointIndex + '-winding-' + windingIndex + '-reflect-button'" v-if="masStore.mas.magnetic.coil.functionalDescription.length == 2 && masStore.mas.inputs.operatingPoints[operationPointIndex].excitationsPerWinding[(windingIndex + 1) % 2] != null" v-tooltip="tooltipsMagneticSynthesisOperatingPoints[(windingIndex == 0? 'reflectPrimary' : 'reflectSecondaries')]" class="btn btn-secondary fs-6 col-2 mt-2 p-0" style="max-height: 1.7em" @click="reflectWinding(windingIndex)">
                        ...
                        <div v-if="!(masStore.mas.magnetic.coil.functionalDescription.length == 2 && masStore.mas.inputs.operatingPoints[operationPointIndex].excitationsPerWinding[(windingIndex + 1) % 2] != null)" class="fs-6 col-2 mt-2 p-0" style="max-height: 1.7em">
                        </div>
                        <button :data-cy="dataTestLabel + '-operating-point-' + operationPointIndex + '-winding-' + windingIndex + '-select-button'" v-tooltip="tooltipsMagneticSynthesisOperatingPoints['editWindingWaveform']" class="btn fs-6 col-2 mt-2 p-0 ms-1" :class="currentWindingIndex == windingIndex? 'btn-success disabled' : masStore.mas.inputs.operatingPoints[operationPointIndex].excitationsPerWinding[windingIndex] != null? 'btn-primary' : 'btn-danger'" @click="changeWinding(windingIndex)" style="max-height: 1.7em;">
                            ...
                            <svg v-else v-if="masStore.mas.inputs.operatingPoints[operationPointIndex].excitationsPerWinding[windingIndex] != null" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                        ...
                        <button :data-cy="dataTestLabel + '-operating-point-' + operationPointIndex + '-import-button'" class="btn btn-secondary fs-6 col-12 mt-2 p-0" style="max-height: 2em" data-bs-toggle="offcanvas" data-bs-target="#ImportOffCanvas" > Import OP
                        </button>
                    </div>
                    <div v-else class="col-12 row m-0 p-0">
                        <button :data-cy="dataTestLabel + '-remove-operating-point-' + operationPointIndex + '-button'" class="btn btn-danger fs-6 col-6 mt-2" @click="removePoint(operationPointIndex)">Remove </button>
                        <button :data-cy="dataTestLabel + '-select-operating-point-' + operationPointIndex + '-button'" class="btn btn-primary fs-6 col-6 mt-2" @click="currentOperatingPointIndex = operationPointIndex">Select </button>
                    </div>
                </div>
